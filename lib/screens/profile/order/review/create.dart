...
class ReviewCreatePage extends GetView<ReviewCreateController> {
  ...
  Widget build(BuildContext context) {
    if (Get.arguments != null) {
      controller.setDefaultValue(Get.arguments);
    }
    return Obx(
      () => Scaffold(
        appBar: CustomAppBar(
          appBar: AppBar(),
          isBack: true,
          title: controller.isWrite ? '리뷰 수정' : '리뷰 쓰기',
          actions: <Widget>[
            TextButton(
              onPressed: () {
                controller.onSubmit();
                if (controller.isWrite) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    CustomSnackBar(title: '리뷰가 정상적으로 수정되었습니다.'),
                  );
                  Get.toNamed('/profile/review');
                } else {
                  ScaffoldMessenger.of(context).showSnackBar(
                    CustomSnackBar(title: '리뷰가 정상적으로 생성되었습니다.'),
                  );
                }
              },
              child: Container(
                    ...
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          NotoText(
                            '구매일: ${baseDateFormat.format(controller.paymentDay)}',
                            size: 14,
                            color: greyColor,
                          ),
                        ],
